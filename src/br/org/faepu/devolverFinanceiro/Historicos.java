...
public class Historicos {
	...
	public void InserirHistorico(BigDecimal usuarioLogado, BigDecimal nuFin, BigDecimal nuNota) throws Exception {
		...
		obs = " devolução de pedidos Taxas Administrativas para Controladoria!";
		...
		try {
			...
			DynamicVO savePar = hisDAO.create()
					...
					.set("USUARIO", nomeUsu)
					.set("DATA", agora)
					.set("ACAO", "DEVOLVER > CONTROLADORIA")
					.set("PROCESSO", "DEVOLUÇÃO TAXAS > CONTROLADORIA")
					.set("OBS", " devolução de pedidos Taxas Administrativas para Controladoria!")
					.set("NUNOTA", nuNota).save();
			
			
		//	SEQ,NUCHAVE,TABELA,USUARIO,DATA,ACAO,PROCESSO,OBS,NUNOTA
		//	VALUES(@SEQ,@FIELD_NUNOTA,'NUNOTA',@USUARIO,@DATA,'IMPOSTOS > ENVIO','VERIFICAÇÃO DE IMPOSTOS',NULL,@FIELD_NUNOTA)
